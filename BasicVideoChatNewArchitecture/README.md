# Basic Video Chat — New Architecture

This sample application shows how to connect to a Vonage Video session, publish a stream, and subscribe to streams using React Native's **New Architecture** (Fabric renderer and TurboModules).

This is the same functionality as the [BasicVideoChat](../BasicVideoChat) sample, but configured with the additional native infrastructure required for React Native's New Architecture.
...
- `ios/<AppName>-Bridging-Header.h` — Imports the Fabric registrar for Swift interop
- `ios/<AppName>/AppDelegate.swift` — Calls `FabricComponentRegistrar.registerCustomComponents()` at launch
